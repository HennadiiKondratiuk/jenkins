pipeline {
    ...
    stages {
        ...
        stage('Pull and Run') {
            steps {
                script{
                    command='''aws ecr get-login-password --region eu-central-1 | docker login --username AWS --password-stdin https://319448237430.dkr.ecr.eu-central-1.amazonaws.com/hkondratiuk-images
                    ...
                    sshagent(credentials: ['Jenkins_agent_ssh_key']) {
                        sh ''ssh ubuntu@10.0.1.116 '${command}' ''
                    }
                }
            }
        }
