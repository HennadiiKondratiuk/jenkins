pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script{
                       sh 'aws ecr get-login-password --region eu-cental-1 | sudo docker login --username AWS --password-stdin https://319448237430.dkr.ecr.eu-central-1.amazonaws.com/hkondratiuk-images'
