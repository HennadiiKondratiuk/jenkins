            ...
            steps {
                script{
                    withCredentials([aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: "aws", secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                        ...
                        sh "aws ecs describe-task-definition --task-definition ${AWS_ECS_TASK_DEFINITION} --region=${AWS_ECR_REGION} | jq '.taskDefinition.containerDefinitions[0].image= \"319448237430.dkr.ecr.eu-central-1.amazonaws.com/hkondratiuk-images:$BUILD_ID\"' > task-def.json"
